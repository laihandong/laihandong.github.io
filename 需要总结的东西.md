...
+ [如何在Linux上搭建Jenkins？自动构建接口测试实践 (qq.com)](https://mp.weixin.qq.com/s/3D4nf0u3AYaf7fg9tr57bQ)
+ [UI性能测试 (qq.com)](https://mp.weixin.qq.com/s/L7vXu1xc1lpNaYYXbAbNDQ)
...
+ [CPU爆了，你却连那个线程出问题都不知道？ (qq.com)](https://mp.weixin.qq.com/s/FvmlnV-oKNxrK5glrFKPHA)
...
<font color=red>**累积的思考和疑惑：**</font>

+ 所写的工具，每次都要重新开启吗？每次都要重新跑一遍所有流程吗？每次都要先读取大量的一系列文件吗？
+ 公共用例多了如何管理?
+ python热更新?
+ 不是在同工程下，使用python调用外部bat文件，只能用绝对路径吗？它不会去电脑（windows）的系统路径下找吗？就一直报错不存在内部或外部命令。。。python里是不是涉及到一个系统路径的继承，当前找不到回到父级项目去找？
+ 



<font color=blue>**临时总结未验证的思考和结论：**</font>

+ unity导入插件，第一次导入可以正常使用，再已导入的基础上再次导入会不会有问题呢？
...
+ 就目前工作遇到的事情，如何管理各项事务的进度，以及记录各个节点：把当前工作&生活想做的事情当成一个个“坑”，俗话说“开坑”嘛，然后每个“坑”都要有至少一个文档进行维护，记录着有关它的所有事情（比如进度、踩坑经验、部署过程、阶段总结、阶段计划、维护计划、bug总结等等）。
...
+ 命令行设置环境变量，linux用eport，windows用set
+ 